(window.webpackJsonp = window.webpackJsonp || []).push([
  [29],
  {
    'Fa/5': function(t, e, n) {
      var r = n('ProS');
      n('y2l5'), n('q/+u');
      var a = n('f5Yq'),
        i = n('h8O9');
      n('Ae16'), r.registerVisual(a('scatter', 'circle')), r.registerLayout(i('scatter'));
    },
    'eTLM': function(t, e, n) {
      'use strict';
      n.d(e, 'a', function() {
        return d;
      });
      var r = n('ofXK'),
        a = n('tyNb'),
        i = n('wZkO'),
        o = n('NFeN'),
        s = n('Wp6s'),
        c = n('sHJK'),
        u = n('ZI6o'),
        l = (n('p6QC'), n('SNPZ'), n('dE5l')),
        h = n('fXoL');
      let d = (() => {
        class t {}
        return (
          (t.ɵmod = h.Qb({ type: t })),
          (t.ɵinj = h.Pb({
            factory: function(e) {
              return new (e || t)();
            },
            providers: [],
            imports: [[r.c, a.f, o.b, s.e, i.f, u.a, c.a, l.a]],
          })),
          t
        );
      })();
    },
    'kpj+': function(t, e, n) {
      'use strict';
      n.r(e);
      var r = n('ofXK'),
        a = n('tyNb'),
        i = n('d3UM'),
        o = n('NFeN'),
        s = n('eTLM'),
        c = n('rDig'),
        u = n('Qk+/'),
        l = n('a4k1'),
        h = n('sZfy'),
        d = n('dE5l'),
        m = n('mrSG'),
        f = n('fXoL'),
        p = n('MpVW'),
        b = n('kmnG'),
        y = n('cvES'),
        g = n('cpAi'),
        v = n('WKB1'),
        w = n('Y0Ja'),
        T = n('Nffh'),
        S = n('FKr1');
      function D(t, e) {
        if ((1 & t && (f.Yb(0, 'mat-option', 11), f.Tc(1), f.Wb()), 2 & t)) {
          const t = e.$implicit;
          f.qc('value', t), f.Db(1), f.Vc('\n        ', t, '\n      ');
        }
      }
      function _(t, e) {
        if (
          (1 & t &&
            (f.Vb(0),
            f.Tc(1, '\n        '),
            f.Yb(2, 'div', 0),
            f.Tc(3, '\n          '),
            f.Yb(4, 'mat-icon'),
            f.Tc(5, 'person'),
            f.Wb(),
            f.Tc(6, '\n          '),
            f.Yb(7, 'span', 13),
            f.Tc(8),
            f.Wb(),
            f.Tc(9, '\n        '),
            f.Wb(),
            f.Tc(10, '\n      '),
            f.Ub()),
          2 & t)
        ) {
          const t = f.kc().$implicit;
          f.Db(4),
            f.Pc('color', t.color),
            f.Db(4),
            f.Vc('\n            ', t.seriesName + ': ' + t.value[1], '\n          ');
        }
      }
      function x(t, e) {
        if ((1 & t && (f.Tc(0, '\n      '), f.Rc(1, _, 11, 2, 'ng-container', 12), f.Tc(2, '\n    ')), 2 & t)) {
          const t = e.$implicit;
          f.Db(1), f.qc('ngIf', t);
        }
      }
      const C = function() {
          return [28604, 77, 17096869, 'Australia', 1990];
        },
        I = function() {
          return [31163, 77.4, 27662440, 'Canada', 1990];
        },
        N = function() {
          return [1516, 68, 1154605773, 'China', 1990];
        },
        P = function() {
          return [13670, 74.7, 10582082, 'Cuba', 1990];
        },
        F = function() {
          return [28599, 75, 4986705, 'Finland', 1990];
        },
        z = function() {
          return [29476, 77.1, 56943299, 'France', 1990];
        },
        K = function() {
          return [31476, 75.4, 78958237, 'Germany', 1990];
        },
        k = function() {
          return [28666, 78.1, 254830, 'Iceland', 1990];
        },
        q = function() {
          return [1777, 57.7, 870601776, 'India', 1990];
        },
        A = function() {
          return [29550, 79.1, 122249285, 'Japan', 1990];
        },
        M = function() {
          return [2076, 67.9, 20194354, 'North Korea', 1990];
        },
        U = function() {
          return [12087, 72, 42972254, 'South Korea', 1990];
        },
        W = function() {
          return [24021, 75.4, 3397534, 'New Zealand', 1990];
        },
        L = function() {
          return [43296, 76.8, 4240375, 'Norway', 1990];
        },
        Y = function() {
          return [10088, 70.8, 38195258, 'Poland', 1990];
        },
        O = function() {
          return [19349, 69.6, 147568552, 'Russia', 1990];
        },
        G = function() {
          return [10670, 67.3, 53994605, 'Turkey', 1990];
        },
        R = function() {
          return [26424, 75.7, 57110117, 'United Kingdom', 1990];
        },
        V = function() {
          return [37062, 75.4, 252847810, 'United States', 1990];
        },
        J = function(t, e, n, r, a, i, o, s, c, u, l, h, d, m, f, p, b, y, g) {
          return [t, e, n, r, a, i, o, s, c, u, l, h, d, m, f, p, b, y, g];
        },
        Z = function() {
          return [44056, 81.8, 23968973, 'Australia', 2015];
        },
        j = function() {
          return [43294, 81.7, 35939927, 'Canada', 2015];
        },
        X = function() {
          return [13334, 76.9, 1376048943, 'China', 2015];
        },
        E = function() {
          return [21291, 78.5, 11389562, 'Cuba', 2015];
        },
        B = function() {
          return [38923, 80.8, 5503457, 'Finland', 2015];
        },
        H = function() {
          return [37599, 81.9, 64395345, 'France', 2015];
        },
        Q = function() {
          return [44053, 81.1, 80688545, 'Germany', 2015];
        },
        $ = function() {
          return [42182, 82.8, 329425, 'Iceland', 2015];
        },
        tt = function() {
          return [5903, 66.8, 1311050527, 'India', 2015];
        },
        et = function() {
          return [36162, 83.5, 126573481, 'Japan', 2015];
        },
        nt = function() {
          return [1390, 71.4, 25155317, 'North Korea', 2015];
        },
        rt = function() {
          return [34644, 80.7, 50293439, 'South Korea', 2015];
        },
        at = function() {
          return [34186, 80.6, 4528526, 'New Zealand', 2015];
        },
        it = function() {
          return [64304, 81.6, 5210967, 'Norway', 2015];
        },
        ot = function() {
          return [24787, 77.3, 38611794, 'Poland', 2015];
        },
        st = function() {
          return [23038, 73.13, 143456918, 'Russia', 2015];
        },
        ct = function() {
          return [19360, 76.5, 78665830, 'Turkey', 2015];
        },
        ut = function() {
          return [38225, 81.4, 64715810, 'United Kingdom', 2015];
        },
        lt = function() {
          return [53354, 79.1, 321773631, 'United States', 2015];
        };
      let ht = (() => {
        class t {
          constructor(t, e) {
            (this._cdr = t),
              (this.themeSelector = e),
              (this.themes = Object(c.e)()),
              (this.config = {
                tooltip: { show: !0 },
                legend: { right: 10, data: ['1990', '2015'] },
                xAxis: [{ show: !0 }],
                yAxis: [{ show: !0 }],
                series: [
                  {
                    name: '1990',
                    data: [
                      [28604, 77, 17096869, 'Australia', 1990],
                      [31163, 77.4, 27662440, 'Canada', 1990],
                      [1516, 68, 1154605773, 'China', 1990],
                      [13670, 74.7, 10582082, 'Cuba', 1990],
                      [28599, 75, 4986705, 'Finland', 1990],
                      [29476, 77.1, 56943299, 'France', 1990],
                      [31476, 75.4, 78958237, 'Germany', 1990],
                      [28666, 78.1, 254830, 'Iceland', 1990],
                      [1777, 57.7, 870601776, 'India', 1990],
                      [29550, 79.1, 122249285, 'Japan', 1990],
                      [2076, 67.9, 20194354, 'North Korea', 1990],
                      [12087, 72, 42972254, 'South Korea', 1990],
                      [24021, 75.4, 3397534, 'New Zealand', 1990],
                      [43296, 76.8, 4240375, 'Norway', 1990],
                      [10088, 70.8, 38195258, 'Poland', 1990],
                      [19349, 69.6, 147568552, 'Russia', 1990],
                      [10670, 67.3, 53994605, 'Turkey', 1990],
                      [26424, 75.7, 57110117, 'United Kingdom', 1990],
                      [37062, 75.4, 252847810, 'United States', 1990],
                    ],
                    type: 'scatter',
                    itemStyle: { opacity: 0.75, color: '#007373' },
                    symbolSize: (t) => Math.sqrt(t[2]) / 500,
                    label: { show: !0, formatter: (t) => t.data[3], position: 'top' },
                  },
                  {
                    name: '2015',
                    data: [
                      [44056, 81.8, 23968973, 'Australia', 2015],
                      [43294, 81.7, 35939927, 'Canada', 2015],
                      [13334, 76.9, 1376048943, 'China', 2015],
                      [21291, 78.5, 11389562, 'Cuba', 2015],
                      [38923, 80.8, 5503457, 'Finland', 2015],
                      [37599, 81.9, 64395345, 'France', 2015],
                      [44053, 81.1, 80688545, 'Germany', 2015],
                      [42182, 82.8, 329425, 'Iceland', 2015],
                      [5903, 66.8, 1311050527, 'India', 2015],
                      [36162, 83.5, 126573481, 'Japan', 2015],
                      [1390, 71.4, 25155317, 'North Korea', 2015],
                      [34644, 80.7, 50293439, 'South Korea', 2015],
                      [34186, 80.6, 4528526, 'New Zealand', 2015],
                      [64304, 81.6, 5210967, 'Norway', 2015],
                      [24787, 77.3, 38611794, 'Poland', 2015],
                      [23038, 73.13, 143456918, 'Russia', 2015],
                      [19360, 76.5, 78665830, 'Turkey', 2015],
                      [38225, 81.4, 64715810, 'United Kingdom', 2015],
                      [53354, 79.1, 321773631, 'United States', 2015],
                    ],
                    type: 'scatter',
                    itemStyle: { opacity: 0.75, color: '#1B98C6' },
                    symbolSize: (t) => Math.sqrt(t[2]) / 500,
                    label: { show: !0, formatter: (t) => t.data[3], position: 'top' },
                  },
                ],
              });
          }
          ngOnInit() {
            return Object(m.a)(this, void 0, void 0, function*() {
              (this.selectedTheme = this.themeSelector.selected), this._cdr.markForCheck();
            });
          }
          selectChartTheme(t) {
            this.themeSelector.select(t);
          }
          symbolSize(t) {
            return Math.sqrt(t[2]) / 500;
          }
        }
        return (
          (t.ɵfac = function(e) {
            return new (e || t)(f.Sb(f.h), f.Sb(p.a));
          }),
          (t.ɵcmp = f.Mb({
            type: t,
            selectors: [['types-scatter']],
            decls: 31,
            vars: 97,
            consts: [
              ['layout', 'row', 'layout-align', 'start center'],
              ['hide-xs', '', 1, 'mat-subheading-2', 'pad-left', 'pad-right', 'push-bottom-none'],
              ['floatLabel', 'never'],
              [3, 'value', 'valueChange'],
              [3, 'value', 4, 'ngFor', 'ngForOf'],
              [3, 'themeName'],
              [3, 'trigger'],
              ['tdTooltipFormatter', ''],
              [3, 'show', 'position', 'type', 'boundaryGap'],
              [3, 'show', 'type'],
              ['td-scatter', '', 3, 'data', 'name', 'symbolSize'],
              [3, 'value'],
              [4, 'ngIf'],
              [1, 'mat-caption', 'pad-left-sm'],
            ],
            template: function(t, e) {
              1 & t &&
                (f.Yb(0, 'div', 0),
                f.Tc(1, '\n  '),
                f.Yb(2, 'div', 1),
                f.Tc(3, '\n    Chart Theme:\n  '),
                f.Wb(),
                f.Tc(4, '\n  '),
                f.Yb(5, 'mat-form-field', 2),
                f.Tc(6, '\n    '),
                f.Yb(7, 'mat-select', 3),
                f.gc('valueChange', function(t) {
                  return (e.selectedTheme = t);
                })('valueChange', function(t) {
                  return e.selectChartTheme(t);
                }),
                f.Tc(8, '\n      '),
                f.Rc(9, D, 2, 2, 'mat-option', 4),
                f.Tc(10, '\n    '),
                f.Wb(),
                f.Tc(11, '\n  '),
                f.Wb(),
                f.Tc(12, '\n'),
                f.Wb(),
                f.Tc(13, '\n'),
                f.Yb(14, 'td-chart', 5),
                f.lc(15, 'async'),
                f.Tc(16, '\n  '),
                f.Yb(17, 'td-chart-tooltip', 6),
                f.Tc(18, '\n    '),
                f.Rc(19, x, 3, 1, 'ng-template', 7),
                f.Tc(20, '\n  '),
                f.Wb(),
                f.Tc(21, '\n  '),
                f.Tb(22, 'td-chart-x-axis', 8),
                f.Tc(23, '\n  '),
                f.Tb(24, 'td-chart-y-axis', 9),
                f.Tc(25, '\n  '),
                f.Tb(26, 'td-chart-series', 10),
                f.Tc(27, '\n  '),
                f.Tb(28, 'td-chart-series', 10),
                f.Tc(29, '\n'),
                f.Wb(),
                f.Tc(30, '\n')),
                2 & t &&
                  (f.Db(7),
                  f.qc('value', e.selectedTheme),
                  f.Db(2),
                  f.qc('ngForOf', e.themes),
                  f.Db(5),
                  f.Pc('height', 300, 'px'),
                  f.qc('themeName', f.mc(15, 17, e.themeSelector.selected$)),
                  f.Db(3),
                  f.qc('trigger', 'item'),
                  f.Db(5),
                  f.qc('show', !0)('position', 'bottom')('type', 'value')('boundaryGap', !0),
                  f.Db(2),
                  f.qc('show', !0)('type', 'value'),
                  f.Db(2),
                  f.qc(
                    'data',
                    f.Dc(38, J, [
                      f.uc(19, C),
                      f.uc(20, I),
                      f.uc(21, N),
                      f.uc(22, P),
                      f.uc(23, F),
                      f.uc(24, z),
                      f.uc(25, K),
                      f.uc(26, k),
                      f.uc(27, q),
                      f.uc(28, A),
                      f.uc(29, M),
                      f.uc(30, U),
                      f.uc(31, W),
                      f.uc(32, L),
                      f.uc(33, Y),
                      f.uc(34, O),
                      f.uc(35, G),
                      f.uc(36, R),
                      f.uc(37, V),
                    ]),
                  )('name', '1990')('symbolSize', e.symbolSize),
                  f.Db(2),
                  f.qc(
                    'data',
                    f.Dc(77, J, [
                      f.uc(58, Z),
                      f.uc(59, j),
                      f.uc(60, X),
                      f.uc(61, E),
                      f.uc(62, B),
                      f.uc(63, H),
                      f.uc(64, Q),
                      f.uc(65, $),
                      f.uc(66, tt),
                      f.uc(67, et),
                      f.uc(68, nt),
                      f.uc(69, rt),
                      f.uc(70, at),
                      f.uc(71, it),
                      f.uc(72, ot),
                      f.uc(73, st),
                      f.uc(74, ct),
                      f.uc(75, ut),
                      f.uc(76, lt),
                    ]),
                  )('name', '2015')('symbolSize', e.symbolSize));
            },
            directives: [b.b, i.a, r.s, y.a, g.a, g.b, v.a, w.a, T.a, S.k, r.t, o.a],
            pipes: [r.b],
            styles: ['[_nghost-%COMP%]{width:100%}'],
            changeDetection: 0,
          })),
          t
        );
      })();
      n.d(e, 'ScatterModule', function() {
        return mt;
      });
      const dt = Object(h.c)({ overviewDemoComponent: ht, id: 'scatter' });
      let mt = (() => {
        class t {}
        return (
          (t.ɵmod = f.Qb({ type: t })),
          (t.ɵinj = f.Pb({
            factory: function(e) {
              return new (e || t)();
            },
            providers: [],
            imports: [[r.c, i.b, o.b, c.a, l.a, u.a, s.a, d.a, a.f.forChild(dt)]],
          })),
          t
        );
      })();
    },
    'q/+u': function(t, e, n) {
      var r = n('ProS'),
        a = n('9wZj'),
        i = n('yWXo'),
        o = n('h8O9');
      r.extendChartView({
        type: 'scatter',
        render: function(t, e, n) {
          var r = t.getData();
          this._updateSymbolDraw(r, t).updateData(r), (this._finished = !0);
        },
        incrementalPrepareRender: function(t, e, n) {
          var r = t.getData();
          this._updateSymbolDraw(r, t).incrementalPrepareUpdate(r), (this._finished = !1);
        },
        incrementalRender: function(t, e, n) {
          this._symbolDraw.incrementalUpdate(t, e.getData()), (this._finished = t.end === e.getData().count());
        },
        updateTransform: function(t, e, n) {
          var r = t.getData();
          if ((this.group.dirty(), !this._finished || r.count() > 1e4 || !this._symbolDraw.isPersistent()))
            return { update: !0 };
          var a = o().reset(t);
          a.progress && a.progress({ start: 0, end: r.count() }, r), this._symbolDraw.updateLayout(r);
        },
        _updateSymbolDraw: function(t, e) {
          var n = this._symbolDraw,
            r = e.pipelineContext.large;
          return (
            (n && r === this._isLargeDraw) ||
              (n && n.remove(),
              (n = this._symbolDraw = r ? new i() : new a()),
              (this._isLargeDraw = r),
              this.group.removeAll()),
            this.group.add(n.group),
            n
          );
        },
        remove: function(t, e) {
          this._symbolDraw && this._symbolDraw.remove(!0), (this._symbolDraw = null);
        },
        dispose: function() {},
      });
    },
    'y2l5': function(t, e, n) {
      var r = n('MwEJ'),
        a = n('T4UG').extend({
          type: 'series.scatter',
          dependencies: ['grid', 'polar', 'geo', 'singleAxis', 'calendar'],
          getInitialData: function(t, e) {
            return r(this.getSource(), this);
          },
          brushSelector: 'point',
          getProgressive: function() {
            var t = this.option.progressive;
            return null == t ? (this.option.large ? 5e3 : this.get('progressive')) : t;
          },
          getProgressiveThreshold: function() {
            var t = this.option.progressiveThreshold;
            return null == t ? (this.option.large ? 1e4 : this.get('progressiveThreshold')) : t;
          },
          defaultOption: {
            coordinateSystem: 'cartesian2d',
            zlevel: 0,
            z: 2,
            legendHoverLink: !0,
            hoverAnimation: !0,
            symbolSize: 10,
            large: !1,
            largeThreshold: 2e3,
            itemStyle: { opacity: 0.8 },
          },
        });
      t.exports = a;
    },
    'yWXo': function(t, e, n) {
      var r = n('IwbS'),
        a = n('oVpE').createSymbol,
        i = n('OS9S'),
        o = r.extendShape({
          shape: { points: null },
          symbolProxy: null,
          buildPath: function(t, e) {
            var n = e.points,
              r = e.size,
              a = this.symbolProxy,
              i = a.shape;
            if (!((t.getContext ? t.getContext() : t) && r[0] < 4))
              for (var o = 0; o < n.length; ) {
                var s = n[o++],
                  c = n[o++];
                isNaN(s) ||
                  isNaN(c) ||
                  ((i.x = s - r[0] / 2),
                  (i.y = c - r[1] / 2),
                  (i.width = r[0]),
                  (i.height = r[1]),
                  a.buildPath(t, i, !0));
              }
          },
          afterBrush: function(t) {
            var e = this.shape,
              n = e.points,
              r = e.size;
            if (r[0] < 4) {
              this.setTransform(t);
              for (var a = 0; a < n.length; ) {
                var i = n[a++],
                  o = n[a++];
                isNaN(i) || isNaN(o) || t.fillRect(i - r[0] / 2, o - r[1] / 2, r[0], r[1]);
              }
              this.restoreTransform(t);
            }
          },
          findDataIndex: function(t, e) {
            for (
              var n = this.shape,
                r = n.points,
                a = n.size,
                i = Math.max(a[0], 4),
                o = Math.max(a[1], 4),
                s = r.length / 2 - 1;
              s >= 0;
              s--
            ) {
              var c = 2 * s,
                u = r[c] - i / 2,
                l = r[c + 1] - o / 2;
              if (t >= u && e >= l && t <= u + i && e <= l + o) return s;
            }
            return -1;
          },
        });
      function s() {
        this.group = new r.Group();
      }
      var c = s.prototype;
      (c.isPersistent = function() {
        return !this._incremental;
      }),
        (c.updateData = function(t) {
          this.group.removeAll();
          var e = new o({ rectHover: !0, cursor: 'default' });
          e.setShape({ points: t.getLayout('symbolPoints') }),
            this._setCommon(e, t),
            this.group.add(e),
            (this._incremental = null);
        }),
        (c.updateLayout = function(t) {
          if (!this._incremental) {
            var e = t.getLayout('symbolPoints');
            this.group.eachChild(function(t) {
              null != t.startIndex &&
                (e = new Float32Array(e.buffer, 4 * t.startIndex * 2, 2 * (t.endIndex - t.startIndex))),
                t.setShape('points', e);
            });
          }
        }),
        (c.incrementalPrepareUpdate = function(t) {
          this.group.removeAll(),
            this._clearIncremental(),
            t.count() > 2e6
              ? (this._incremental || (this._incremental = new i({ silent: !0 })), this.group.add(this._incremental))
              : (this._incremental = null);
        }),
        (c.incrementalUpdate = function(t, e) {
          var n;
          this._incremental
            ? ((n = new o()), this._incremental.addDisplayable(n, !0))
            : (((n = new o({
                rectHover: !0,
                cursor: 'default',
                startIndex: t.start,
                endIndex: t.end,
              })).incremental = !0),
              this.group.add(n)),
            n.setShape({ points: e.getLayout('symbolPoints') }),
            this._setCommon(n, e, !!this._incremental);
        }),
        (c._setCommon = function(t, e, n) {
          var r = e.hostModel,
            i = e.getVisual('symbolSize');
          t.setShape('size', i instanceof Array ? i : [i, i]),
            (t.symbolProxy = a(e.getVisual('symbol'), 0, 0, 0, 0)),
            (t.setColor = t.symbolProxy.setColor);
          var o = t.shape.size[0] < 4;
          t.useStyle(r.getModel('itemStyle').getItemStyle(o ? ['color', 'shadowBlur', 'shadowColor'] : ['color']));
          var s = e.getVisual('color');
          s && t.setColor(s),
            n ||
              ((t.seriesIndex = r.seriesIndex),
              t.on('mousemove', function(e) {
                t.dataIndex = null;
                var n = t.findDataIndex(e.offsetX, e.offsetY);
                n >= 0 && (t.dataIndex = n + (t.startIndex || 0));
              }));
        }),
        (c.remove = function() {
          this._clearIncremental(), (this._incremental = null), this.group.removeAll();
        }),
        (c._clearIncremental = function() {
          var t = this._incremental;
          t && t.clearDisplaybles();
        }),
        (t.exports = s);
    },
  },
]);
